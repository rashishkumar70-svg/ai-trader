...
# ══════════════════════════════════════════════════════════════════
# 📡 UPCREDITS · UPSTOX LIVE QUOTES — optional real-time layer over Yahoo
...
#   candle — states & alerts react in seconds, not at candle close.
#   Anything fails / no token → silent Yahoo fallback. Zero crash risk.
#   Credentials live ONLY inside this app instance (or Streamlit Secrets)
...
def up_load():
    ...
    try:
        ...
        pass
    try:   # Streamlit-Cloud Secrets (survive redeploys) — optional
        s = st.secrets
        d["key"] = d["key"] or str(s.get("UPSTOX_KEY", "") or "")
        d["secret"] = d["secret"] or str(s.get("UPSTOX_SECRET", "") or "")
        d["redirect"] = d["redirect"] or str(s.get("UPSTOX_REDIRECT", "") or "")
    except Exception:
        pass
    return d
...
def up_settings_ui(tag=""):
    with st.expander("📡 UPSTOX LIVE QUOTES (optional — real-time prices)"):
        ...
        if k.strip() and sc.strip():
            ...
            if st.session_state.get(f"up_show{tag}"):
                ...
                if st.button("🚀 Activate live quotes", key=f"up_go{tag}", **STRETCH):
                    ...
                    if ok:
                        st.success("🟢 LIVE! Real-time quotes are ON for today — every scan now uses "
                                   "tick-fresh prices.")
                        try:
        ...
        st.caption("Free Upstox Basic plan · quotes only · no orders · no funds. Keys + token stay inside "
                   "your app instance (file or Streamlit Secrets) — never in GitHub. Any failure = silent "
                   "Yahoo fallback, scans never break.")
